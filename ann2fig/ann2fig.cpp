...
//				[-sz size] [-dx dim_x] [-dy dim_y] [-sl dim
//value]*
//				[-ps pointsize]
//				file
//
//		where:
//			-upi scale			fig units per inch (default =
//1200) 			-x low_x			x and y offset of upper left corner
//(inches) 			-y low_y			...(default = 1) 			-sz size
//maximum side length of figure (in inches)
//								...(default = 5)
//			-dx dim_x			horizontal dimension (default =
//0) 			-dy dim_y			vertical dimension (default = 1) 			-sv value
//default slice value (default = 0)
//			-sl dim value		each such pair defines the value
//along the
//								...given dimension at which to
//slice.  This
//								...may be supplied for all
//dimensions except
//								...dim_x and
//dim_y. 			-ps pointsize		size of points in fig units (def = 10) 			file
...
//
//		writeHeader				write header for fig
//file 		writePoint				write a point 		writeBox
//write a box 		writeLine				write a line
//----------------------------------------------------------------------
...
void recReadTree(ANNorthRect& box)
{
    ...
    { // leaf node
        ...
        if (dim == 2 || overlap(box))
        {
            ...
            { // yes, write the points
                ifile >> idx;
                writePoint(pts[idx]);
            }
        }
        else
        { // input but ignore points
            for (int i = 0; i < n_pts; i++)
            {
                ifile >> idx;
            }
    ...
    { // splitting node
        ...
        if (dim == 2 || overlap(box))
        { // check for overlap
            if (cd == dim_x || cd == dim_y)
            {                              // cut through slice plane
...
//		points <dim> <n_pts>			(point coordinates: this is
//optional) 		0 <xxx> <xxx> ... <xxx>			(point indices and
//coordinates) 		1 <xxx> <xxx> ... <xxx>
//		  ...
//		tree <dim> <n_pts> <bkt_size>
//		<xxx> <xxx> ... <xxx>			(lower end of bounding
//box) 		<xxx> <xxx> ... <xxx>			(upper end of bounding box) 				If
//the tree is null, then a single line "null" is 				output.  Otherwise the nodes of
//the tree are printed 				one per line in preorder.  Leaves and splitting nodes
...
//				values "-INF" and "+INF", respectively.  We do
//not 				check for this, because the current version of ANN 				starts with a finite
//bounding box if the tree is 				nonempty.
//----------------------------------------------------------------------
