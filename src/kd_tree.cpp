...
//constructing 		the kd-tree (see Friedman, Bentley, and Finkel, ``An algorithm
//for 		finding best matches in logarithmic expected time,'' ACM Transactions 		on
//Mathematical Software, 3(3):209-226, 1977).  The procedure 		operates by a
//simple divide-and-conquer strategy, which determines 		an appropriate orthogonal
//cutting plane (see below), and splits 		the points.  When the number of points
//falls below the bucket size, 		we simply store the points in a leaf node's
//bucket.
//
...
//
//				void split(
//						ANNpointArray pa,  // complete point
//array 						ANNidxArray pidx,  // point array (permuted on return) 						ANNorthRect
//&bnds, // bounds of current cell 						int n,			   // number of
//points 						int dim,		   // dimension of space 						int &cut_dim, //
//cutting dimension 						ANNcoord &cut_val, // cutting value 						int &n_lo)
//// no. of points on low side of cut
//
