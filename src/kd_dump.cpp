...
//the tree is null, then a single line "null" is 				output.	 Otherwise the
//nodes of the tree are printed 				one per line in preorder.  Leaves and splitting
//nodes 				have the following formats: 		Leaf node: 				leaf <n_pts> <bkt[0]> <bkt[1]>
//... <bkt[n-1]> 		Splitting nodes: 				split <cut_dim> <cut_val> <lo_bound>
//<hi_bound>
//
//		For bd-trees:
//
...
//dump 		file contains all the basic tree information according to a 		preorder
//traversal.	 We assume that the dump file also contains 		point data. (This is
//to guarantee the consistency of the tree.) 		If not, then an error is generated.
//
            ...
            ANNpoint& the_bnd_box_hi) // high bounding point (ret'd)
{
    ...
    //annReadTree 			to do all the heavy work.  We create our own array of 			point
    //indices (so we can pass them to annReadTree()) 			but we do not deallocate
    //them.	They will be deallocated 			when the tree is destroyed.
    //------------------------------------------------------------------
...
//internal). 		It returns a pointer to the node that was created.	An array 		of
//point indices is given along with a pointer to the next 		available location in
//the array.  As leaves are read, their 		point indices are stored here, and the
//point buckets point 		to the first entry in the array.
//
