...
//x-bisector, 		then the split is taken along the maximum x-coordinate of the 		data
//points.
//
                    ...
                    int& n_lo) // num of points on low side (returned)
{
    ...
    //and 	if ideal_cut_val > max (implying br1 <= n-1), 			then we select n_lo =
    //n-1 (so there is one point on right). 	Otherwise, we select n_lo as close
    //to n/2 as possible within 			[br1..br2].
    //------------------------------------------------------------------
...
//midpt_split, 		but is more complicated as well.  When point distribution is
//		extremely skewed, this degenerates to midpt_split (actually
//		1/3 point split), and when the points are most evenly
//distributed, 		this degenerates to kd-split.
//----------------------------------------------------------------------
...
//cut 		lies between these extreme cuts, then we use the median cut. 		If not, then
//consider the extreme cut that is closer to the 		median.	 If all the
//points lie to one side of this cut, then 		we slide the cut until it hits the
//first point.	 This may 		violate the aspect ratio bound, but will never
//generate empty 		cells.	However the sibling of every such skinny cell is fat,
                   ...
                   int& n_lo) // num of points on low side (returned)
{
    ...
    ANNcoord lo_cut = bnds.lo[cut_dim] + small_piece; // lowest legal cut
    ANNcoord hi_cut =
        bnds.hi[cut_dim] - small_piece; // highest legal cut
