...
//
//		Nodes in kd-trees are of two types, splitting nodes which
//contain 		splitting information (a splitting hyperplane orthogonal to one 		of the
//coordinate axes) and leaf nodes which contain point 		information (an array of
//points stored in a bucket).  This is 		handled by making a generic class
//kd_node, which is essentially an 		empty shell, and then deriving the leaf and
//splitting nodes from 		this.
//----------------------------------------------------------------------
...
//		kd_splitter is a pointer to a splitting routine for
//preprocessing. 		Different splitting procedures result in different strategies
...
//		to box distance calculations) [we do not store the entire
//bounding 		box since this may be wasteful of space in high dimensions]. 		We also
//store pointers to the 2 children.
//----------------------------------------------------------------------
