    ...
    while (getDirective(cin, directive))
    {
        ...
        else if (!strcmp(directive, "gen_data_pts"))
        {
            ...
            { // planted distribution
                Error("Cannot use planted distribution for data points",
                      ANNabort);
            }
        ...
        else if (!strcmp(directive, "gen_query_pts"))
        {
            ...
            { // planted distribution
                if (data_pts == NULL)
                {
                    Error("Must generate data points before query points for "
                          "planted distribution",
                          ANNabort);
                }
        ...
        //
        //		**	input the argument (standard or priority) and
        //output 			the header describing the essential information.
        //		**	allocate space for the results to be stored.
        //		**	run the queries by invoking the appropriate
        //search 			procedure on the query points.  Print nearest neighbor 			if
        //requested.
        ...
        else if (!strcmp(directive, "run_queries"))
        {
            ...
            {
                Error("Either data set and query set not constructed",
                      ANNabort);
            }
            if (the_tree == NULL)
            {
...
void getTrueNN() // compute true nearest neighbors
{
    if (stats > SILENT)
    {
        cout << "(Computing true nearest neighbors for validation.  This may "
                "take time.)\n";
    }
    // deallocate existing storage
    if (true_nn_idx != NULL)
    ...
    if (true_nn > data_size)
    { // can't get more nn than points
        true_nn = data_size;
    }
