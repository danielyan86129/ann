...
//ANNpointSet. 		Cleaned up C++ structure for modern compilers 	Revision 1.1
//05/03/05 		Added fixed-radius k-NN searching 	Revision 1.1.2  01/27/10 		Fixed
//minor compilation bugs for new versions of gcc
//----------------------------------------------------------------------
...
//quantization,'' 				Proc. of DCC '93: Data Compression Conference, eds. J. A.
...
//searching,'' 				5th Ann. ACM-SIAM Symposium on Discrete Algorithms, 				1994,
//573-582.
//----------------------------------------------------------------------
...
//points. 			Note that this routine is not used internally by ANN for 			computing
//distance calculations.  For reasons of efficiency 			this is done using
//incremental distance calculation.  Thus, 			this routine cannot be modified as a
//method of changing the 			metric.
//
...
//				Allocate a deallocate storage for a single point,
//and 				return a pointer to it.  The argument to AllocPt() is 				used to initialize
//all components.
//
//		annAllocPts() and annDeallocPts():
//				Allocate and deallocate an array of points as well
//a 				place to store their coordinates, and initializes the 				points to point to
//their respective coordinates.  It 				allocates point storage in a contiguous
//block large 				enough to store all the points.  It performs no 				initialization.
//
//		annCopyPt():
//				Creates a copy of a given point, allocating space
//for 				the new point.  It returns a pointer to the newly 				allocated copy.
//----------------------------------------------------------------------
...
//ANNbd_tree 		A bd-tree tree search structure (a kd-tree with shrink
//		capabilities).
//
...
//visiting 				the closer child first. 			Priority search (annkPriSearch()): 				Searches
//nodes in order of increasing distance of the 				associated cell from the query
//point.  For many 				distributions the standard search seems to work just 				fine,
//but priority search is safer for worst-case 				performance.
//
//		Printing:
//		---------
...
//
//		dim						Dimension of
//space 		n_pts					Number of points currently in
//the tree
//		n_max					Maximum number of points that
//are allowed 								in the tree
//		bkt_size				Maximum bucket size (no. of points
//per leaf) 		bnd_box_lo				Bounding box low point
